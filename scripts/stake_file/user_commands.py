...
class UserCommands:
    def __init__(self):
        # CORRIGIDO: Não é mais necessário passar 'config' para CompositionRoot.
        # CompositionRoot agora acessa CONFIG diretamente.
...
def create_user(name: str, email: str):
    ...
    async def _run_create():
        try:
            ...
            # Garante que o dispose do engine ocorra no MESMO loop
            await SQLUnitOfWork.dispose_engine()
...
@click.option('--name', prompt='New user name (leave empty to keep current)', default='', show_default=False)
@click.option('--email', prompt='New user email (leave empty to keep current)', default='', show_default=False)
def update_user(user_id: int, name: str, email: str):
    """Update an existing user."""
    commands = UserCommands()
    async def _run_update():
        try:
            result = await commands.update_user_async(user_id, name if name else None, email if email else None)
            click.echo(result)
        finally:
            await SQLUnitOfWork.dispose_engine()
            StructuredLogger.shutdown()
