...
# Bibliotecas como `pydantic` ou `environs` poderiam ser usadas para garantir tipos e valores obrigatórios.
...
class Configuration:
    ...
    def _load_environment_variables(self):
        ...
        if os.path.exists(full_dotenv_path):
            ...
            if self._environment == "production":
                print(
                    f"AVISO: Arquivo .env.{self._environment} não encontrado em {full_dotenv_path}. Assumindo que as variáveis de ambiente são configuradas externamente para produção."
                )
            else:
                warnings.warn(
    ...
    def _load_config_file(self):
        ...
        if os.path.exists(full_config_file_path):
            try:
                ...
                warnings.warn(
                    f"Erro ao carregar o arquivo de configuração {full_config_file_path}: {e}"
                )
        else:
            print(
                f"INFO: Arquivo de configuração {full_config_file_path} não encontrado. Usando apenas variáveis de ambiente e padrões."
            )
            pass
    ...
    def database_url(self) -> str:
        ...
        if not url:
            raise ConfigurationException(
                "DATABASE_URL is not configured for the current environment."
            )
